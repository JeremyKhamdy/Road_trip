...
import {getAll, getEatDetails} from "@/services/EatServices";
...
export default defineComponent({
  ...
  computed: {
    cssProps() {
      return {
        '--img-url': 'url(/src/assets/images/restaurants.jpg)'
      }
    }
  },
  setup() {
    ...
    const pageName = ref("Restaurants");
    ...
    onMounted(async () => {
      ...
      if (typeof isInitForm !== "undefined" && isInitForm.value !== 0) {
        getEatDetails(form.destinations, 0, typesValue.value, priceMinValue.value, priceMaxValue.value).then((res) => {
          console.log(res);
          let data = res.data;
    ...
    const ratingFilters = (rating: number) => {
      getEatDetails(form.destinations, rating, typesValue.value.value, priceMinValue.value, priceMaxValue.value).then((res) => {
        let data = res.data;
    ...
    const priceFilters = (min: number, max: number) => {
      getEatDetails(form.destinations, ratingValue.value, typesValue.value.value, min, max).then((res) => {
        let data = res.data;
        results.splice(0, results.length);
        countResult.value = 0;
    ...
    const typeFilters = (checkbox: []) => {
      ...
      getEatDetails(form.destinations, ratingValue.value, checkbox.value, priceMinValue.value, priceMaxValue.value).then((res) => {
        let data = res.data;
        results.splice(0, results.length);
        countResult.value = 0;
    ...
    <section class="search-section">
      <div class="last-search" v-if="isInitForm === 0">
        ...
        <div class="grid-cards">
          <Card :page-name="pageName" v-for="(result, index) in results.slice(0, 8)" :key="index"></Card>
        </div>
      </div>
