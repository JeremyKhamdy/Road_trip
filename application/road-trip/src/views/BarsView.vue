...
import {getAll, getDrinksDetails} from "@/services/DrinkServices";
...
export default defineComponent({
  name: "Bars",
  components: {Card, Search, HeaderCommon, Header},
  computed: {
    cssProps() {
      return {
        '--img-url': 'url(/src/assets/images/bars.jpg)'
      }
    }
  },
  setup() {
    ...
    onMounted(async () => {
      ...
      if (typeof isInitForm !== "undefined" && isInitForm.value !== 0) {
        getDrinksDetails(form.destinations, 0, typesValue.value, priceMinValue.value, priceMaxValue.value).then((res) => {
          let data = res.data;
    ...
    const ratingFilters = (rating: number) => {
      getDrinksDetails(form.destinations, rating, typesValue.value.value, priceMinValue.value, priceMaxValue.value).then((res) => {
        let data = res.data;
    ...
    const priceFilters = (min: number, max: number) => {
      getDrinksDetails(form.destinations, ratingValue.value, typesValue.value.value, min, max).then((res) => {
        let data = res.data;
        results.splice(0, results.length);
        countResult.value = 0;
    ...
    const typeFilters = (checkbox: []) => {
      ...
      getDrinksDetails(form.destinations, ratingValue.value, checkbox.value, priceMinValue.value, priceMaxValue.value).then((res) => {
        let data = res.data;
        results.splice(0, results.length);
        countResult.value = 0;
