...
const api = {
    ...
    eatDetails: async (city, rating, types, priceMin, priceMax): Promise<any> => {
        let queryPrice = "";
        ...
        const eat = await fetch(
            `https://maps.googleapis.com/maps/api/place/textsearch/json?query=category=food+${city}${queryPrice}&type=establishment&key=${API_KEY}`
        );
    ...
    eatDetail: async (placeID): Promise<any> => {
        const eat = await fetch(
