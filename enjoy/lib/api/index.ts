...
const api = {
    ...
    enjoyDetails: async (city, category, rating, types, priceMin, priceMax): Promise<any> => {
        let queryPrice = "";
        ...
        console.log(city);
        console.log(`https://maps.googleapis.com/maps/api/place/textsearch/json?query=${category}${city}${queryPrice}&type=establishment&key=${API_KEY}`);
        const eat = await fetch(
            // `https://maps.googleapis.com/maps/api/place/textsearch/json?query=${category}+${city}${queryPrice}&type=establishment&key=${API_KEY}`
            `https://maps.googleapis.com/maps/api/place/textsearch/json?query=${category}${city}${queryPrice}&key=${API_KEY}`
        );
    ...
    enjoyDetail: async (placeID): Promise<any> => {
        const eat = await fetch(
