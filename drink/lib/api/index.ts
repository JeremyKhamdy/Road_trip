...
const api = {
    ...
    drinkDetails: async (city, rating, types, priceMin, priceMax): Promise<any> => {
        let queryPrice = "";
        ...
        const eat = await fetch(
            `https://maps.googleapis.com/maps/api/place/textsearch/json?query=category=bar+${city}${queryPrice}&type=establishment&key=${API_KEY}`
        );
    ...
    drinkDetail: async (placeID): Promise<any> => {
        const eat = await fetch(
